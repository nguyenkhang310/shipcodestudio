...
import { MessageCircle, FileText, Code2, TestTube2, Presentation, ArrowRight, CheckCircle2 } from "lucide-react";
...
const steps = [
  { 
    ...
    details: ["Phân tích nghiệp vụ", "Đề xuất stack công nghệ", "Chốt phạm vi và tính năng", "Ký kết lộ trình rõ ràng"] 
  },
  { 
    icon: FileText, 
    ...
    details: ["Thiết kế Database (ERD)", "Sơ đồ kiến trúc hệ thống", "Mock-up UI/UX Figma", "Chốt thiết kế với khách"] 
  },
  { 
    icon: Code2, 
    ...
    details: ["Clean code & Best practices", "Quản lý source bằng Git", "Demo tiến độ hàng tuần", "Review và tinh chỉnh"] 
  },
  { 
    icon: TestTube2, 
    ...
    details: ["Unit & Integration Test", "Tối ưu hiệu năng (Performance)", "Viết tài liệu HDSD", "Hướng dẫn triển khai"] 
  },
  { 
    icon: Presentation, 
    ...
    details: ["Deploy lên Server/Cloud", "Training vận hành hệ thống", "Bàn giao tài liệu", "Bảo hành, bảo trì dài hạn"] 
  },
];
...
export const ProcessContent = () => {
  return (
    <>
      <div className="relative ml-4 space-y-8 pb-8 md:ml-8 md:space-y-10">
        {/* Animated gradient timeline line */}
        <div className="absolute left-0 top-0 bottom-0 w-px overflow-hidden">
          <motion.div
            className="w-full h-full"
            style={{
              background: "linear-gradient(180deg, hsl(42 92% 58% / 0.5), hsl(358 62% 48% / 0.3), hsl(190 95% 58% / 0.2), transparent)",
              transformOrigin: "top",
            }}
            initial={{ scaleY: 0 }}
            ...
            <div className="premium-card group p-4 sm:p-6 transition-all duration-300 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10 md:p-7">
              <div className="mb-4 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                ...
                <motion.div
                  ...
                  whileHover={{ scale: 1.05, backgroundColor: "hsl(42 92% 58% / 0.2)" }}
                >
                  {s.time}
                </motion.div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {s.details.map((d, dIdx) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium group-hover/detail:text-foreground transition-colors">{d}</span>
                  </motion.div>
                ))}
              </div>
            </div>
      ...
      >
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
          <Button asChild variant="hero" size="xl" className="group text-lg px-10 shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-shadow">
            <Link to="/lien-he">
              Bắt đầu Bước 01 <ArrowRight className="ml-2 transition-transform group-hover:translate-x-2" />
            </Link>
          </Button>
        </motion.div>
      </motion.div>
  ...
  return (
    <Page>
      <PageHeader
        index="02"
        eyebrow="Quy trình"
        title={<>5 bước, <span className="text-gradient">0 lo lắng.</span></>}
